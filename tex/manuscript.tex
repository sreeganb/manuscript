...
combined with window based sampling for free energy calculations. \cite{Peters08JAmChemSoc130p17342,Beckham10epsbook} 
...
simulations. For the MAT2A enzyme the substrates were modified to 
L-methionine and adenosyl triphosphate (ATP) whereas for the \textit{pv}ADA
...
using potassium ions. Both the systems were minimized with 50 steps of the
...
equilibration to prepare the systems for production runs.  
%------------------------------------------------------------------------------ 
\subsection{Transition path sampling}
%------------------------------------------------------------------------------
We used TPS to generate an ensemble of reactive trajectories. 
The time step that we used for all our calculations was $1\;fs$ with a total
of 250 steps forward and 250 steps backward which amounts to a total of $0.5\;ps$
trajectory.   
\subsubsection{human MAT2A}
The key bond parameters for the reaction in this system consists of the 
oxygen atom on the ATP, the carbon atom on the adenosyl group and the 
sulphur atom on the L-methionine. The reactant basin is defined by the 
oxygen-carbon bond distance being less than 1.7 {\AA} and the product basin 
is characterized by the suphur-carbon bond distance being less than 
1.7 {\AA}. The initial reactive trajectory was generated by the application 
of a bias harmonic force between the oxygen and the carbon atoms 
and running dynamics for $250\;fs$. The shooting algorithm was used to generate
new trajectories within which first a random time slice is chosen from the reactive 
trajectory and perturbing the momenta of the system. Then with the new momenta
the system is propagated forward and backward in time for $250\;fs$.
A total of 120 reactive trajectories were generated. 

\subsubsection{plasmodium vivax ADA}
The $pv$ADA reaction is 
%------------------------------------------------------------------------------ 
