...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[journal=jpcb,manuscript=article,reprint]{achemso}
...
Atomistic level detailed study of the mechanism of enzyme catalysis 
reactions is necessary to understand their efficiency as well as to 
design molecules that can act as inhibitors for their activity. 
Molecular dynamics (MD) simulations 2 have often been used to study 
large biomolecular systems in atomistic detail since they provide an 
excellent computational cost to detail ratio. MD simulations to study 
chemical reactions that go through free energy barriers are often 
impractical to simulate since it would take a long time for the system 
to cross over barriers.
To overcome these energy barriers and 
to obtain ergodic sampling several methods have been developed over the years such as 
...
milestoning, etc. These methods rely on applying bias
potentials for the system to overcome energy barriers and hence require the knowledge of 
the reaction coordinates.  
Transition path sampling \cite{Bolhuis02AnnRevPhysChem53p291,Dellago98JChemPhys108p1964}
(TPS) is an enhanced sampling method for 
rare events that transition a system between two stable states often 
referred to as reactants and products. 6

TPS focuses on transition paths and not on stable states which introduces 
biases in the sampling of equilibrium distribution of the reaction coordinate. 

TPS based free energy simulations have been attempted previously by Brotzakis et al., \cite{Brotzakis19JChemPhys151p174111}

One example of the TPS based free energies calculations on real protein systems \cite{Silveira21JPhysChemB125p2018}

Human Mat2a enzyme and plasmodium vivax adenosine deaminase \cite{Luo07JAmChemSoc129p8008,Ho09Biochemistry48p9618} 
\section{Theory}
...
CHARMM force field \cite{Brooks09JComputChem30p1545}

\section{Adenosine deaminase reaction coordinates and free energy}
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.25]{./figures/ada-protein.png}
\end{figure}
