...
TPS can be considered as a generalization of the Monte Carlo sampling of the 
configuration space to the space of trajectories where the collected ensemble of 
reactive pathways are real dynamical trajectories compared to articial trajectories
generated using bias forces. The TPS ensemble can be used to obtain kinetic information
such as the rate of the reaction as well as the transition state, reaction
coordinate and the reaction mechanism. TPS has been successfully 
applied to study problems such as enzyme catalysis, protein folding, etc. 

Since TPS focuses on collecting only reactive trajectories the information contained 
in the non-reactive trajectories which is essential for describing the 
equilibrium distribution is missing. 
Methods that can be used to simultaneously calculate
the dynamical as well as equilibrium properties of a system within 
a certain set of asssumptions have garnered interest in recent times. \cite{Suarez14JChemTheoryComput10p2658}
Transition interface sampling (TIS) is a variant of TPS which can be used for 
this purpose. \cite{vanErp03JChemPhys118p7762}

A common theme among such methods is to divide the reaction coordinate into
windows within which independent sampling can be performed and interpolated
in a way that can used to calculate equilibrium properties

To calculate equilibrium properties such as the free energy 
